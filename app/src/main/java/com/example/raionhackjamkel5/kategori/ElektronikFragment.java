...
public class ElektronikFragment extends Fragment {
    ...
    ArrayList<ElektronikModel> elektronikItems;
    ElektronikAdapter elektronikAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_elektronik, container, false);

        recyclerView = view.findViewById(R.id.rvElektronikKategori);
    ...
    private void showElektronikData(){
        database.child("Kategori").child("Elektronik").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                elektronikItems = new ArrayList<>();
                for (DataSnapshot item : snapshot.getChildren()) {
                    ElektronikModel katalog = item.getValue(ElektronikModel.class);
                    katalog.setKey(item.getKey());
                    elektronikItems.add(katalog);
                }
                Collections.reverse(elektronikItems);
                elektronikAdapter = new ElektronikAdapter(elektronikItems, getContext());
