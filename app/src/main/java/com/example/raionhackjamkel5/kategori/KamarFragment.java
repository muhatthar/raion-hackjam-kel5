...
import com.example.raionhackjamkel5.kategoriAdapter.KamarAdapter;
...
public class KamarFragment extends Fragment {
    ...
    ArrayList<KamarModel> kamarItems;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_kamar, container, false);

        recyclerView = view.findViewById(R.id.rvKamarKategori);
    ...
    private void showKamarData(){
        database.child("Kategori").child("Kamar").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                kamarItems = new ArrayList<>();
                for (DataSnapshot item : snapshot.getChildren()) {
                    KamarModel katalog = item.getValue(KamarModel.class);
                    katalog.setKey(item.getKey());
                    kamarItems.add(katalog);
                }
                Collections.reverse(kamarItems);
                kamarAdapter = new KamarAdapter(kamarItems, getContext());
